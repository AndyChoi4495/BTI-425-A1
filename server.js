...
app.get('/api/movies', (req, res) => {
  db.getAllMovies(req.query.page, req.query.perPage, (req.query.title = null))
...
app.get('/api/movies/:id', (req, res) => {
  db.getMovieById(req.params.id)
    .then((data) => {
      data
        ? res.json(data)
        : res.status(404).json({ message: 'cannot find movie id' });
    })
    .catch((err) => {
...
app.put('/api/movies/:id', (req, res) => {
  db.updateMovieById(req.body, req.params.id)
    .then((data) => {
      data
        ? res.json(data).json({ message: 'Movie is Updated' })
        : res.status(404).json({ message: 'movie is not updated' });
    })
    .catch((err) => {
...
app.delete('/api/movies/:id', (req, res) => {
  db.deleteMovieById(req.params.id)
    .then((data) => {
      data
        ? res.status(204).end()
        : res.status(404).json({ message: 'Can not delete' + req.params.id + '!' });
    })
    .catch(() => {
